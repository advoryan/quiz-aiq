(function(){
  const comp = {
    ...
    emits:['update:answer'],
    data(){ return { id:'q1', text:'What is your favorite color?', options:['Red','Blue','Green','Other'] }; },
    template:`<section :class="$root.panelClasses(stepNumber)" v-show="true" class="space-y-6 bg-white/90 rounded-xl shadow-sm ring-1 ring-slate-200/70 p-5 sm:p-8">
      <h2 class=\"text-2xl font-semibold tracking-tight\">Question 1 of {{$root.questions.length}}</h2>
      <div class=\"space-y-4\">
        ...
        <div class=\"flex justify-between pt-4\">
          <button type=\"button\" @click=\"$root.prevStep()\" class=\"inline-flex items-center justify-center px-5 py-2.5 rounded-lg font-medium bg-white text-slate-700 border border-slate-300 hover:bg-slate-50 focus:outline-none focus:ring-4 focus:ring-primary/20 transition\" :disabled=\"$root.step === $root.firstQuestionStep\">Back</button>
  ...
  (window.__questionComponents = window.__questionComponents || []).push(['question-q1', comp]);
})();
