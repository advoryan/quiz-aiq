(function(){
  const comp = {
    ...
    emits:['update:answer'],
    data(){ return { id:'q2', text:'Do you own a home?', options:['Yes','No'] }; },
    template:`<section :class="$root.panelClasses(stepNumber)" v-show="true" class="space-y-6 bg-white/90 rounded-xl shadow-sm ring-1 ring-slate-200/70 p-5 sm:p-8">
      <h2 class=\"text-2xl font-semibold tracking-tight\">Question 2 of {{$root.questions.length}}</h2>
      <div class=\"space-y-4\">
  ...
  (window.__questionComponents = window.__questionComponents || []).push(['question-q2', comp]);
})();
